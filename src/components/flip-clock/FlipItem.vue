...
$rate: 1;
$width: calc($rate * 60px);
$height: calc($rate * 90px);
$fontSize: calc($rate * 80px);
$lineWidth: calc($rate * 3px);
$radius: calc($rate * 6px);

.flip {
  position: relative;
  margin: calc($rate * 5px);
  width: $width;
  height: $height;
  font-size: $fontSize;
  font-weight: bold;
  ...
  box-shadow: 0 calc($rate *1px) calc($rate * 10px) rgba(0, 0, 0, 0.7);
  ...
  .item {
    ...
    .inn {
      ...
      height: 200%;
      color: #ccc;
      text-shadow: 0 calc($rate * 1px) calc($rate * 2px) #000;
      text-align: center;
